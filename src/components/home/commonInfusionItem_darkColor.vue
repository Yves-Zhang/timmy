<template>
  <div class="darkColorouterWarp" :style="isFull && itemStyle" @click="() => goToDetail(item.bagId, item.status)">
    <div class="content" :class="item.statusClassName" >
        <!-- <div class="bedNumberBox" ><span class="bedNumber">{{item.bedCode}}</span></div> -->
            ...
            <div class="infusionBagBox" :style="isFull && infusionBagBox">
              <div class="infusionBag  bottleState" :class="{'alarmBed':item.justShowBedCode}" :style="{'border':item.justShowBedCode?'2px #F85982 solid':'none','background':item.justShowBedCode?'#fff':'','height':item.justShowBedCode?'calc( 100% - 4px)':''}">
                <div class="remainFluidText" v-if="item.status === 'I' && item.showText" :style="isFull && remainFluidText">
                  ...
                  <span v-show="!item.speedAlarm">
                    <i class="icon iconfont icon-icons_thelast" style="font-size: 12px;font-weight: bold;"></i>
              ...
              <div class="textDebail" :style="isFull && textDebail">
                <dl class="patient-detail">
                  ...
                  <dd>
                    ...
                    <span>{{item.patientAge}} </span><span> {{item.inHospitalNo}}</span>
                  </dd>
                  <dd>
                    {{item.admitDate}}
                  </dd>
                  <dd class="tendLevel" :style="{'color':tendLevel[item.tendLevel]}">
                    {{levelText(item.tendLevel)}}
                  </dd>
                </dl>
              </div>
            </div>
...
<script>
require('./commonInfusionItem_darkColor.scss');
...
export default {
  ...
  name: 'CommonInfusionItemDarkColor',
