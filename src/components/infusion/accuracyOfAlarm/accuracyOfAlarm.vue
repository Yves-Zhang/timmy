...
<div>
  <el-card class="box-card infusion-overview">
    <div slot="header" class="card-header">
      <div class="card-title"><i class="icon iconfont icon-icons_infusion_stati"></i> 报警准确率趋势图</div>
    </div>
...
export default {
  created() {
    this.getData();
    let date=moment().subtract(30, 'days')._d;
  },
  mounted() {
  ...
  methods: {
    ...
    getData() {
      ...
      return this.$http.get(`${CONST.PATH}/statInfusionInfos`, {
          params,
        }).then(response => {
          if (response.body) {
            this.loading = false;
    ...
    drawChart() {
      ...
      if (this.queryParams.statModel !== 'byDay') {
        for (let _i = startI; _i <= i; _i += 1) {
          ...
          keyingSpeed.push(0);
          accuracyRate.push(0);
          scanbarcodeRate.push(0);
          infusionBagNum.push(0);
        }
      }
      ...
      data.forEach(item => {
        if (this.queryParams.statModel === 'byDay' && typeof item.natural_stat_date !== 'undefined') {
          ...
          keyingSpeed.push(item.presskey_rate);
          accuracyRate.push(item.accuracy_rate);
          scanbarcodeRate.push(item.scanbarcode_rate);
          infusionBagNum.push(item.infusion_bag_num);
                
        } else if (typeof item[key] !== 'undefined') {
          const existIndex = _.findIndex(categories, cate => parseInt(cate.replace('月'), 10) === parseInt(item[key], 10));
          accuracyRate[existIndex] = item.accuracy_rate;
          keyingSpeed[existIndex] = item.presskey_rate;
          scanbarcodeRate[existIndex] = item.scanbarcode_rate;
          infusionBagNum[existIndex] = item.infusion_bag_num;
